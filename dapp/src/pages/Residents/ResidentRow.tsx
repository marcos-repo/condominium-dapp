...
function ResidentRow(props: Props) {
    ...
    function getNextPayment() {
        const dateMs = ethers.toNumber(props.data.nextPayment) * 1000;
        ...
        return (
            <span className={"text-xs mb-0 ms-3 " + color}>
                {text}
            </span>
        );
    ...
    function btnDeleteClick() {
        if(confirm("Confirma a exclusão deste morador?")){
        ...
        <>
            <tr>
                ...
                <td className="align-middle text-center">
                    <span className="text-secondary text-xs font-weight-bold">{getNextPayment()}</span>
                </td>
